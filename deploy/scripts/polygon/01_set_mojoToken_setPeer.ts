...
import { LZ_ETHEREUM_ENDPOINT_ID, LZ_ETHEREUM_MOJO_TOKEN_ADDRESS, LZ_POLYGON_MOJO_TOKEN_ADDRESS } from "../../mainnet-constants";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const peer = ethers.utils.hexZeroPad(LZ_ETHEREUM_MOJO_TOKEN_ADDRESS, 32);
  
  const tx = await token.setPeer(LZ_ETHEREUM_ENDPOINT_ID, peer);
  await tx.wait(1);
